...
# extended for moz internal use by David Joslin
...
# check if installed, install if not - DJ
if [ ! -e $MegaCli ]; then
...
# Programatically define enclosure - DJ
ENCLOSURE=`$MegaCli -PDlist -a0|grep "Enclosure Device ID:"|head -n1|cut -d" " -f4`
...
if [ $1 = "drives" ] || [ $1 = "disks" ]
   then
      ...
      $MegaCli -PDlist -aALL -NoLog > $FILE
      for DRIVE in $(grep ^Slot $FILE|cut -d" " -f3); do
        sed -n -e "/Slot Number: $DRIVE$/,/Enclosure Device ID:/ p" $FILE|sed -e '$d' > $FILE"_DRIVE_"$DRIVE
        ERRORS=`grep Count: $FILE"_DRIVE_"$DRIVE|awk '{sum+=$4} END {print sum}'`
        SMART=`grep S.M.A $FILE"_DRIVE_"$DRIVE|awk '{print $NF}'`
        TYPE=`grep ^Firmware $FILE"_DRIVE_"$DRIVE| cut -d' ' -f3-`
        if [ "$TYPE" != "JBOD" ]; then
          DISKGROUP=`grep DiskGroup  $FILE"_DRIVE_"$DRIVE| cut -d' ' -f3-`
        else
          DISKGROUP="n/a"
        fi
        printf "%-8s | %-6s | %-10s | %-15s | %-30s\n" "Slot $DRIVE" "$ERRORS" "$SMART" "$TYPE" "$DISKGROUP"
        rm $FILE"_DRIVE_"$DRIVE
      done
      rm $FILE
   exit
fi
